...
const storage = createStorage({ booking_form: JSON.stringify({ sender_name: 'Legacy Person' }), booking_step: '3' });
...
const formA = { route: 'Manila → Bohol', sender_name: 'Customer A', receiver_phone: '09170000000' };
...
persistBookingDraft(userB, { sender_name: 'Customer B' }, 2, storage);
...
assert.equal(readBookingDraft(userB, storage).form.sender_name, 'Customer B', 'logout does not indiscriminately clear other scoped state');
