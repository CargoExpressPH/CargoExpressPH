...
const orderPaymentRealtime = read('src/hooks/useOrderPaymentRealtime.js');
const returnContext = read('src/lib/paymentReturnContext.js');
...
assert.match(customerOrderDetail, /useOrderPaymentRealtime\(id, refreshOrderSilently\)/);
assert.match(adminOrderDetail, /useOrderPaymentRealtime\(id, \(\) => loadOrder\(true, \{ silent: true \}\)\)/);
assert.match(orderPaymentRealtime, /table: 'orders'/);
assert.match(orderPaymentRealtime, /window\.addEventListener\('online'/);
assert.match(orderPaymentRealtime, /visibilitychange/);
assert.match(returnContext, /candidate\.startsWith\('\/'\)/);
assert.match(returnContext, /candidate\.startsWith\('\/\/'\)/);
...
assert.match(paymentReturn, /You may now close this tab and return to the device where you started your payment\./);
assert.match(paymentReturn, /window\.location\.replace\(originatingReturnTo\)/);
assert.match(paymentReturn, /window\.close\(\)/);
assert.doesNotMatch(paymentReturn, /PaymentResultModal|trackingNumber|paymentMethod|useAuth/);
