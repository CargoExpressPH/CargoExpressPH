-- ============================================================
-- Admin-only shipping discount — reporting (get_sales_summary)
--
-- Full body copied from 20260806040000_outstanding_single_source.sql (the
-- latest definition) with three changes, all additive:
--
--   1. `outstanding` in the active_orders CTE now subtracts discount_amount
--      too — it is read five times downstream (outstandingTotal,
--      outstandingAllOrders, unpaidCount, unpaidOrders, monthlySales[].
--      outstanding), so fixing it here fixes all five at once, the same way
--      this CTE already keeps the Sales tab and the Unsettled tab agreeing.
--
--   2. `total_revenue` (both the all-time summary figure and each month's) is
--      now NET of discount — shipping_cost minus discount_amount — matching
--      "final fees ... must use discounted amounts" for what this report
--      calls revenue. It still means "what is billed", not "what is
--      collected" (that is paidTotal / ledgerTotal, unchanged: both are
--      already pure sums over payment_transactions, which a discount never
--      touches).
--
--   3. A new `totalDiscounts` field — SUM(discount_amount) — so gross fee
--      before discount stays recoverable (totalRevenue + totalDiscounts) and
--      an admin can see how much was given away, without redesigning the
--      report shape. No existing JSON key is renamed or removed, so a
--      frontend bundle that has not picked up the new field yet keeps working
--      unchanged (see the deployment-order note in the implementation report).
-- ============================================================
...
BEGIN
  ...
  WITH active_orders AS (
    SELECT
      ...
      -- Discount-aware: what is owed is the DISCOUNTED fee minus what has
      -- been paid, never the pre-discount shipping_cost.
  ...
  order_rollup AS (
    SELECT
      -- Net of discount: what is actually billed, not the pre-discount ask.
