-- ============================================================
-- Migration: 20260922170000_person_name_policy_order_updates.sql
-- Purpose  : Apply public.is_valid_person_name() (added in
--            20260922160000_person_name_policy.sql) to every UPDATE of an
--            existing booking's sender/receiver name.
--
-- guard_order_update() is a BEFORE UPDATE trigger on public.orders, so it
-- sees the RPC path (update_order_contact_details()) and a raw client
-- `.update()` alike — the same reason 20260915130000 moved the contact lock
-- down here rather than leaving it in the RPC. Enforcing the name policy in
-- the RPC alone would have left the raw-update path open.
--
-- The function is reproduced VERBATIM from
-- 20260922100000_split_sender_receiver_names.sql apart from the single block
-- marked "NEW (20260922160000)", which is inserted between the first/last
-- <-> full-name sync and the contact/address lock.
--
-- LEGACY DATA IS NOT BLOCKED AND NOT REWRITTEN. Every added check is gated
-- on the column actually changing. Nothing in this migration updates a row.
-- ============================================================
...
-- ============================================================
-- VERIFY (an unrelated update to a legacy row must still succeed):
--   UPDATE public.orders SET status = status WHERE id = '<legacy order>';
-- ============================================================
