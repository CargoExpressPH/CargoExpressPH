...
|---|---|---|---|
| 1 | `get_sales_overview_data()` (the "Sales Overview" tab) bucketing bug. | **Medium-High** | Confirmed (Test 2 verified SQL mismatch). Fixed via migration `20260918113805_fix_sales_overview_refund_period.sql`. |
| 2 | ~~Changing the company's price-per-kilo setting can retroactively re-price...~~ **CORRECTED:** A "tripless Picked Up" order is structurally impossible due to the `orders_trip_required_for_active_status` CHECK constraint. | **None (Invalid)** | Disproven by Test 1 (database CHECK blocks this state). |
| 3 | ~~A customer can, in theory, rewrite the text of an admin's chat message...~~ **CORRECTED:** A BEFORE UPDATE trigger explicitly blocks content changes while allowing `is_read` updates. | **None (Invalid)** | Disproven by Test 3 (trigger logic confirmed secure). |
